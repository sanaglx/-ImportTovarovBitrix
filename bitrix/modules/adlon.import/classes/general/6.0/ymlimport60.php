...
class CYmlImport60
{
    ...
    var $FILE_POS=0;
    var $params = Array(); //массив свойств (пока для offers)
        ...
        if (strlen($strImportErrorMessage) <= 0)
        {
            ...
            foreach($CategiriesList as $Categoria){
                ...
                $CATEGORIA_XML_ID = 'yml_'.$Categoria['id'];
                    ...
                    }//Если родитель не указан - пусть катгория идет в корень
 ...
 /*******************************************************************************************************************/
                			//связующим ест массив $connect
                			//$connect[$code] - код параметра выгрузки
                			//$connect[$code]["code"] - код свойства элемента
                			//$connect[$code]["type"] - тип свойства элемента
                            //$connect[$code]["value"] - ИД свойства элемента
                            //$connect[$code]["list"] - список значений для свойства типа L
                                if($improvproperty):
                                    $connect = Array(); //формируем массив соотвествий 
                                    //Г.К.если расширенный режим импорта установлен то устанавливает выбранные типы для свойств 
                                    $createnew = false; //загрузка с расширенного режима с выбором параметров
                                    ...
                                    if($CONNECTIONS)
                                    {
                                        ...
                                        foreach($CONNECTIONS as $i => $cc)
                                        {
                                                ...
                                                $connect[$code]["list"] = Array();//обнуляем спискок для свойст типа лист
                                                
                                                //устанавливаем был ли задан тип
                                                if($cc["type"] != '')                                       
                                                    $connect[$code]["type"] = $cc["type"];
                                                else
                                                    $connect[$code]["type"] = "S";  
                                                        
                                                $prpId = $cc["value"]; //ид свойства

                                                //1)Если свойство уже существует
                                                if($prpId > 0)
                                                {
                                                    //устанавливаем связь
                                                    ...
                                                    //2)новое свойство, не задана привязка                                          
                                                    //проверяем есть ли уже свойство с таким кодом, если есть, то задаем ему другой код 
                                                    $arrCode = array();
                                    ...
                                    }else $createnew = true; //загрузка в те свойства что есть или совпадают по коду, либо создание новых свойств
                                endif;
...
/*******************************************************************************************************************/                   
                                ...
                                if($improvproperty):
                                    foreach($xOfferListNode->param as $param)
                                    {
                                        ...
                                        if(!in_array($code,$params)){                                       
                                            $params[] = $code;
                                            //Г.К.ищем по коду и добавляем свойство в инфоблок                                      
                                            ...
                                            if($PropID == 0)
                                            {
    											...
    											if($createnew)
    											{
    												$connect[$code]["code"] = $code;										
    												$connect[$code]["type"] = "S"; 
    												//если не были заданы входные параметры с выбором свойст (обычный режим загруки)
    												//ищем по коду свойство
    											...
    											if($PropID == 0){ //если нет такого свойства, добавляем новое
    									...
    									/****************************************************************/
    									//перед записью проверяем набор ЛИСТ параметров. 
    									//если нет значения в наборе то добавляем его	  
    									if($connect[$code]["type"] == "L")
    									{
    										...
    										{															
    											if(!in_array(iconv('utf-8',SITE_CHARSET,$param), $connect[$code]["list"]))
    											{
    												...
    												//здесь так сделано чтоб сохранить ИД записей в листе. 
    												$addparam = true;
    									...
    									//Г.К.Записываем значение свойства в товар									
	...
	//                              print $param['name']."<br>";
									...
									if($improvproperty):
    									...
    									/**************************свойство для главного товара *************************************/	
    									if(!in_array($code,$params)) //добавляем новое свойство
    									{
    										   		$params[] = $code;
    										  			
    										        $PropID = intVal($connect[$code]["value"]);	
    										  		   	
    									         	//проверяем и записываем новое свойство
                            ...
                            //упрощенный вариант - ищем по названию свойство, если нет, то добавляем в инфоблок
